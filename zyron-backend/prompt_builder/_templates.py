"""
_templates.py
Per-domain, per-role expert prompt templates + dispatcher.

Ported from src/agents/prompts/promptTemplates.js:
  - CODING_TEMPLATES, MATH_TEMPLATES, WRITING_TEMPLATES
  - ANALYTICAL_TEMPLATES, CREATIVE_TEMPLATES
  - FINANCIAL_TEMPLATES, LEGAL_TEMPLATES
  - GENERAL_TEMPLATES, SIMPLE_TEMPLATES
  - get_expert_template()
"""
...
2. **Architecture decision record (ADR)** — chosen pattern + rejected alternatives with one-line rationale each.
...
3. **Axioms and assumptions** — the exact conditions required for the derivation to hold.
...
6. **Significant figures discipline** — final answer uses the fewest sig figs of any input.
7. **Sanity check** — verify: correct order of magnitude? Physical sign correct? Limiting behavior sensible?
...
Your output MUST include:
1. **Graph description** — describe the key graph in detail: x-axis label + unit, y-axis label + unit, curve shape, intercepts, inflection points, asymptotes, and what each region means physically.
2. **Physical mechanism in plain language** — explain WHY the equation behaves as it does. Not "the formula shows" but "physically, this happens because..."
...
4. **Phase diagram / regime map** — identify at least 2 distinct behavioral regimes. What are the transition conditions?
5. **Everyday analogy** — one concrete, testable analogy from daily life. Explicitly state where the analogy breaks down.
6. **Numerical intuition anchors** — 2–3 real-world reference values that help calibrate the magnitude of the answer.
...
WRITING_TEMPLATES: dict[str, str] = {
    "reasoner": """
...
4. **Structural blueprint** — hook type (anecdote / statistic / provocation / question / paradox) → body organization → closing type.
...
3. **Concrete specificity throughout** — replace every abstraction with real numbers or named examples.
...
6. **Alternative variant** — a meaningfully different version of the key section (different hook type, structural approach, or tone register).""",
...
Your output MUST include:
1. **Line-level surgery** — identify the 5 weakest lines in the draft. For each: quote the line, diagnose the specific failure, then rewrite it.
2. **Rhythm and cadence audit** — find 2 passages where the sentence rhythm fights the meaning. Prescribe a structural fix.
3. **Emotional arc mapping** — trace the emotional journey sentence by sentence. Where does it peak? Where does tension deflate prematurely? Prescribe one structural move to fix the arc.
4. **Precision vocabulary upgrade** — identify 6 generic or overused words and replace with the mot juste.
5. **Cut list** — identify content that must be deleted: throat-clearing preamble, redundant restatements, hedging language, tangential asides.
6. **Opening and closing verdict** — grade the first sentence and last sentence independently. Rewrite either that fails.""",
}

# ─── Analytical domain ────────────────────────────────────────────────────────

ANALYTICAL_TEMPLATES: dict[str, str] = {
    "reasoner": """
...
Your output MUST include:
1. **Precise definition** — define every key term with the rigor of a philosophy of language paper.
2. **Claim decomposition** — break the central claim into its sub-claims. Evaluate each independently.
...
4. **Strongest opposing argument** — steelman the counterposition. The best possible version of the opposing view. Then dismantle it specifically.
...
6. **Epistemic confidence levels** — for each claim: high confidence (proven), medium (probable), low (plausible).
7. **Second-order effects** — what does accepting this conclusion imply about other adjacent beliefs?""",
...
Your output MUST include:
1. **Evidence hierarchy** — categorize evidence by type and strength: meta-analysis > RCT > observational > case study > expert consensus > first principles.
2. **Comparative matrix** — compare at minimum 3 alternatives across consistent criteria. Every criterion must be independently evaluated, not bundled.
...
4. **Quantified trade-off analysis** — for each option: magnitude of benefit, magnitude of cost, probability of success, implementation timeline.
5. **Decision framework** — provide a decision tree or if-then logic for which option fits which context. "It depends" is never an acceptable endpoint.
6. **Confidence-adjusted recommendation** — a direct, specific recommendation with explicit caveats.""",
...
2. **Mental model construction** — design one durable mental model: a spatial, causal, or narrative framework the reader can return to.
...
4. **Progressive disclosure architecture** — design the reading order as a dependency tree. What must be understood before what?
5. **Insight crystallization** — identify the single "aha" sentence that, once grasped, makes everything else obvious. State it in one sentence of maximum 20 words.
6. **Retention anchor** — design one mnemonic, visual hook, or memorable phrase that makes this idea impossible to forget.""",
}

# ─── Creative domain ──────────────────────────────────────────────────────────

CREATIVE_TEMPLATES: dict[str, str] = {
    "reasoner": """
...
TENSION → Every memorable piece exploits a tension. What is the tension in this brief?
...
4. **Anti-brief** — explicitly state what this piece must NOT do. Include at least 3 specific executional traps.
5. **Unexpected format recommendation** — propose one unconventional structural or formal approach. Explain the creative risk and payoff.
...
Your output MUST include:
1. **Three hook variants** — one conventional (safe but well-executed), one subversive (challenges the reader's assumption), one formally unusual. Label each and explain the creative choice.
2. **Full draft** — complete the piece, not a summary of it. No placeholders.
3. **Sensory density** — identify 3 moments in the draft where you deliberately added sensory specificity. Explain what each one does for the reader.
4. **The unexpected element** — one thing in the piece that most writers wouldn't include. Point to it explicitly.
...
Your output MUST include:
1. **The single strongest element** — the line, image, or idea that justifies the piece's existence. Quote it exactly. Explain with precision why it works.
2. **The single weakest element** — the moment where the piece loses the reader. Quote it. Diagnose the failure. Rewrite it.
3. **Tonal consistency audit** — identify every moment where the voice shifts register unexpectedly. Rule: is the shift intentional and effective, or is it a lapse? Prescribe a fix for lapses.
4. **Rhythm surgery** — find one passage where the sentence rhythm is working against the content. Perform a specific structural intervention.
5. **Six precision word swaps** — find 6 instances of a generic, overused, or imprecise word. For each: quote the current word in context, name the failure mode, provide the replacement word.
6. **Opening/closing verdict** — the first sentence and last sentence must each do one irreplaceable job. Evaluate each. If not doing that job, rewrite.""",
}
...
Your output MUST include:
1. **Question reframing** — state the deeper question beneath the surface question.
...
5. **Closing strength check** — the final sentence must be either an action, a memorable insight, or an open question that compels thought.""",
}

# ─── Simple-mode focus directives ────────────────────────────────────────────

_SIMPLE_TEMPLATES: dict[str, dict[str, str]] = {
    "coding": {
        "reasoner": """## Your Focus
...
def get_expert_template(role: str, primary_type: str, verbosity_level: str = "simple") -> str:
    """
    Returns the expert template string for a given role, query type, and verbosity.

    verbosity_level == 'simple'   → plain, focused instructions (default)
    verbosity_level == 'detailed' → full academic/research-level mandates

    Always returns a non-empty string (falls back to general).
    """
    if verbosity_level == "detailed":
        advanced_map: dict[str, dict[str, str]] = {
        ...
        templates = advanced_map.get(primary_type, GENERAL_TEMPLATES)
        return templates.get(role) or GENERAL_TEMPLATES.get(role, "")

    # Simple mode (default)
    simple_map = {
        "coding":     _SIMPLE_TEMPLATES["coding"],
        "stem":       _SIMPLE_TEMPLATES["stem"],
        "writing":    _SIMPLE_TEMPLATES["writing"],
        "analytical": _SIMPLE_TEMPLATES["analytical"],
        "creative":   _SIMPLE_TEMPLATES["creative"],
        "financial":  _SIMPLE_TEMPLATES["financial"],
        "legal":      _SIMPLE_TEMPLATES["legal"],
    }
    templates = simple_map.get(primary_type, _SIMPLE_TEMPLATES["general"])
    return templates.get(role) or _SIMPLE_TEMPLATES["general"].get(role, "")
